 ...
 * Muestra la tarjeta de debito virtual, datos basicos y estadisticas del mes.
 */
class AcmeTarjeta extends HTMLElement {
    ...
    calcularEstadisticasMes() {
        ...
        const ingresos = txs
            .filter(t => String(t.tipo || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase() === 'consignacion')
    ...
    generarNumeroTarjeta() {
        const numeroTarjeta = String(this.cuenta.numeroTarjetaVirtual ?? '').replace(/\D/g, '').padStart(16, '0').slice(0, 16);
        return numeroTarjeta.replace(/(\d{4})/g, '$1 ').trim();
    }
    ...
    render() {
        ...
        const movimientosHTML = txsRecientes.length === 0
            ...
            : txsRecientes.map(t => {
                const esIngreso = String(t.tipo || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase() === 'consignacion';
                return `
        ...
        this.innerHTML = `
            ...
            <div style="max-width: 820px; margin: 0 auto;">
                ...
                <p style="color: var(--text-light); margin-bottom: 1.5rem;">Debito Banco Acme · Cuenta de ahorros</p>

                <div class="tarjeta-virtual ${tarjetaBloqueada ? 'bloqueada' : ''}">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.5rem;position:relative;z-index:1;">
                        ...
                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.45rem;">
                            <span style="font-size:0.75rem;opacity:0.7;">Debito</span>
                            <span class="estado-tarjeta ${tarjetaBloqueada ? 'bloqueada' : ''}">
                                ${tarjetaBloqueada ? 'Tarjeta bloqueada' : 'Tarjeta activa'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="tarjeta-footer">
                        ...
                        <div>
                            <div class="tarjeta-label">Valida hasta</div>
                ...
                <div class="acciones-tarjeta">
                    <button class="accion-btn" id="btn-toggle-numero">
                        ...
                        <span>${this.mostrarNumero ? 'Ocultar' : 'Ver numero'}</span>
                    </button>
                    ...
                    <button class="accion-btn" id="btn-bloquear">
                        <span class="accion-icono"></span>
                        <span>${tarjetaBloqueada ? 'Desbloquear' : 'Bloquear'}</span>
                    </button>
                ...
                <h4 style="margin-bottom: 0.75rem;">Estadisticas - ${mes.charAt(0).toUpperCase() + mes.slice(1)}</h4>
                <div class="stat-row">
                    <div class="stat-box">
                ...
                <div class="card" style="margin-top: 0.5rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem;">
                        <h4 style="margin:0;">Ultimos movimientos</h4>
    ...
    addEventListeners() {
        ...
        this.querySelector('#btn-copiar-cuenta')?.addEventListener('click', () => {
            navigator.clipboard.writeText(this.cuenta.numeroCuenta).then(() => {
                this.mostrarAlerta(this.querySelector('#alerta-tarjeta'), `Numero de cuenta ${this.cuenta.numeroCuenta} copiado al portapapeles.`, 'success');
            }).catch(() => {
                this.mostrarAlerta(this.querySelector('#alerta-tarjeta'), `Cuenta: ${this.cuenta.numeroCuenta}`, 'success');
            });
        });
        ...
        this.querySelector('#btn-bloquear')?.addEventListener('click', () => {
            ...
            const mensaje = nuevoEstado
                ? 'Has bloqueado tu tarjeta con exito.'
                : 'Has desbloqueado tu tarjeta con exito, ya puedes usarla nuevamente.';
            this.mostrarAlerta(this.querySelector('#alerta-tarjeta'), mensaje, 'success');
        });
